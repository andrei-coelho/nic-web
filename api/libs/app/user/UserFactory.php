    ...
    public static function generate_by_session(string $session, string $slug_service, string $slug_function){
        ...
        _query(
            "SELECT 
                ...
                user_client.ghost          as user_ghost,
                session.expire             as session_expire,
                act_serv_func_reg.template as template,
                service_function.id        as service_function_id,
                user.admin,
            ...
            FROM user 
                JOIN session ON session.user_id = user.id
                    ...
                    LEFT JOIN act_serv_func_reg ON service_function.id  = act_serv_func_reg.service_function_id
